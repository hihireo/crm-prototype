...
const ExcludeDebtsModal = ({
  ...
  return (
    <div className="scl-exclude-overlay" onClick={onClose}>
      <div
        className="scl-exclude-modal"
        role="dialog"
        aria-modal="true"
        aria-labelledby="scl-exclude-title"
    ...
    <div className="scl-asset-block">
      ...
      {hasCollateral && (
        <div className="scl-collateral-equity">
          <span>
            시가 {valueMan.toLocaleString()} − 담보{" "}
            {balanceMan.toLocaleString()}
          </span>
  ...
  consultations: [
    {
      ...
      category: "재상담",
      content:
        "소득 증빙 서류 추가 제출 예정. 사채 3천만원은 금융기관 대출이 아닌 점 참고 요망.",
    ...
    },
    {
      ...
      category: "결제유력",
      content:
        "고객 이번 주 내 계약 의사 있음. 개인회생 방향으로 진행 검토 중.",
...
const SampleChecklistPage = () => {
  ...
  const clearAssetCollateral = (assetId) => {
    ...
    if (hasInput && !window.confirm("이 자산의 담보 대출을 삭제할까요?"))
      return;
    setForm((p) => ({
      ...p,
      debts: ensureRows(p.debts.filter((d) => d.collateralAssetId !== assetId)),
    }));
  };
  ...
  const enteredDebts = sortDebtsForDisplay(form.debts).filter(
    (d) => d.lender || (parseInt(d.principal) || 0) > 0,
  );
                  ...
                  <>
                    ...
                    <div className="scl-income-summary">
                      ...
                      <div className="scl-income-row total">
                        <span>월 가용 소득</span>
                        <strong
                          className={repaymentCapacity < 30 ? "warn" : ""}
                        >
                          {repaymentCapacity >= 0 ? "+" : ""}
